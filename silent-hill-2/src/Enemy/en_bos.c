...
#define ENEMY_BOS_ANIME_5801     5801
#define ENEMY_BOS_ANIME_5802     5802
#define ENEMY_BOS_ANIME_5803     5803
#define ENEMY_BOS_ANIME_5804     5804
#define ENEMY_BOS_ANIME_5805     5805
#define ENEMY_BOS_ANIME_5806     5806
#define ENEMY_BOS_ANIME_5808     5808
#define ENEMY_BOS_ANIME_5807     5807
#define ENEMY_BOS_ANIME_5809     5809
...
// size: 0x28, address: 0x2A9A60
extern /* static */ EnANIME_DATA EnBOSAnime[10] = {
    { ENEMY_BOS_ANIME_5801, false, },
    { ENEMY_BOS_ANIME_5801, true,  },
    { ENEMY_BOS_ANIME_5802, false, },
    { ENEMY_BOS_ANIME_5803, false, },
    { ENEMY_BOS_ANIME_5804, true,  },
    { ENEMY_BOS_ANIME_5805, false, },
    { ENEMY_BOS_ANIME_5806, false, },
    { ENEMY_BOS_ANIME_5808, false, },
    { ENEMY_BOS_ANIME_5807, false, },
    { ENEMY_BOS_ANIME_5809, true   },
};

// @note inlines needed for match
static inline float enBOSSize(/* no void */) {
    return 250.0f;
}
static inline float enBOSTall(/* no void */) {
    return 800.0f;
}
...
void enBOSInitData(EnLOCAL_DATA* dp /* r17 */) {
    ...
    enSetSize(dp, enBOSSize(), enBOSTall(), 100.0f, -150.0f);
    dp->weight = 2;
    dp->flag |= (1 << 10) | (1 << 1);
    if (mode < 3) {
...
static void enBOSCtrlAutomatic(EnLOCAL_DATA* dp /* r16 */) {
    ...
    enBOSAnimeExec(dp);
    if (dp->slv < ENEMY_BOS_STATE_DEAD) {
        enBOSMoveExec(dp);
        enBOSSoundSigns(dp);
...
static void enBOSCtrlApproach(EnLOCAL_DATA* dp /* r16 */) {
    ...
    if (enBOSCheckDamage(dp) != 2) {
        ...
        vu0_add_vector(pos, enGetPlayerPos(dp), vec);
        if (enCheckPlayerHitEyes(dp, pos) >= 0.0f) {
            vu0_sub_vector(pos, enGetPlayerPos(dp), vec);
        }
        ...
        switch (enBOSCanAttackPlayer(dp)) {
            ...
            default:
                if (dp->p_dist < 1500.0f || !enCheckIntoScreen(dp)) break;
                if (dp->bos.insect_dp) {
                    if (((EnLOCAL_DATA*)dp->bos.insect_dp)->kind != NULL) break;
...
static void enBOSCtrlAttack(EnLOCAL_DATA* dp /* r16 */) {
    ...
    if (enCheckDamage(dp)) {
        if (enCheckHuggedPlayer()) {
            ...
            if (enBOSCheckDamage(dp) == 2) return;
            ENEMY_STEP(ENEMY_BOS_STATE_DISTANCE);
            return;
        }
    }
    ...
    switch (dp->sslv) {
        ...
        case ENEMY_BOS_STATE_DISTANCE:
            ...
            vu0_sub_vector(dp->vec, vec, &dp->scp->pos);
            dp->vec[3] = 0.0f;
            dp->vec[1] = 0.0f;
            enAttackCheck(dp, 59);
            if (dp->anim_n != -1) return;
            if (dp->flag & (1 << 2)) {
                enBOSAnimeSet(dp, 6);
                ENEMY_SUB_STEP(4);
                return;
...
static void enBOSCtrlDead(EnLOCAL_DATA* dp /* r16 */) {
    if (dp->scp->en_first_status == 13) {
        if (dp->sslv == ENEMY_BOS_STATE_DISTANCE) {
        ...
        if (dp->sslv == ENEMY_BOS_STATE_APPROACH) {
            enSetTimer(dp, 60);
            enSetBlur();
            dp->flag |= 1 << 3;
            ENEMY_NEXT_SUB_STEP();
...
// size: 0x10, address: 0x2A9AF0
static float bed_pos[4] = {
    58900.0f,
    -16000.0f,
    58800.0f,
    800.0f,
...
// size: 0x10, address: 0x2A9B00
static float stair_pos[4] = {
    60900.0f,
    -16000.0f,
    55600.0f,
    900.0f,
...
static void enBOSMoveExec(EnLOCAL_DATA* dp /* r17 */) {
    ...
    enMoveAngle(&tpath, TO_RAD(1));
    dp->scp->rot.y  = tpath.angle;
    if (mode != 0) {
        s = (-17500.0f - (dp->scp->pos).y) * 0.2f;
    } else {
        s = shSway1f(sway, 0.1f);
    }
    dp->bos.y_speed = ((dp->bos.y_speed + s) - ((dp->scp->pos).y - (-17500.0f)) * 0.01f) * 0.98f;
...
// @note inline needed for match
static inline float enBOSSoundVolume(void) {
    return 0.4f;
}
...
static void enBOSSoundSigns(EnLOCAL_DATA* dp /* r16 */) {
    ...
    if ((dp->bos.count -= shGetDF()) <= 0) {
        ...
        dp->bos.signs = signs;
        if (signs) {
            enSoundCall(signs + ENEMY_BOS_SIGNS_SOUND, enBOSSoundVolume(), &dp->scp->pos);
            return;
        }
        enSoundCall3D(ENEMY_BOS_SIGNS_SOUND, enBOSSoundVolume(), &dp->scp->pos);
    }
}
